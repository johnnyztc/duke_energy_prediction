...
db = db.tail(200)
db2 = pd.read_sql_query('SELECT * FROM duke.prediction',engine)
db2 = db2.set_index('Datetime')
...
class Prediction(Base):
    ...
    prediction = Column(Numeric(12,2))

db = db.set_index('Datetime')

db1 = db.tail(200)

future = pd.date_range(db1.index[-1] + timedelta(hours=1),db1.index[-1] + timedelta(hours=24),freq = '1h')
...
df = df[['avg_temp','prediction']]
if df.index[-1] <= db2.index[-1]:
    print('There is nothing to update')
else:
    df = df[df.index>db2.index[-1]]
    df = df.reset_index()
    ...
    for i in range(df.shape[0]):
        ...
        a1 = df.iloc[i,1]
        a2 = df.iloc[i,2]
        item_name = Prediction(Datetime=a0, avg_temp=a1, prediction=a2)
